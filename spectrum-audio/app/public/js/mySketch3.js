...
function setup() {
	...
	// keyPressed();
	// keyCode = UP_ARROW;
	capturer = new CCapture({ format: 'png' , framerate: 24} );
	capturer.start();
...
function draw() {
	...
	console.log("capture consume time is "+(Date.now()-startTime)+" mills")
}
...
function keyPressed() {
	...
	if(key == 'R'){strokeCap(ROUND); rec = 0;capturer.stop();
    	capturer.save();} // Circles
